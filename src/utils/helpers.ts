...
export const hasMovie = (movies: Array<IMovie>, imdbId: string): boolean =>
    movies && movies.length > 0 ? movies.some(x => x['imdbID'] === imdbId) : false

export const setMovies = (movies: Array<IMovie>, movie: IMovie): Array<IMovie> => {
    if (!movie) return movies

    const isFavorite = hasMovie(movies, movie['imdbID'])
